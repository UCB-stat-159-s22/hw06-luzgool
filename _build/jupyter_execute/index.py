...
# utility functions
from ligotools.utils import whiten, write_wavfile, reqshift, create_plot
...
for det in dets:
    ...
    if make_plots:
        ...
        plt.figure(figsize=(10,8))
        plt.subplot(2,1,1)
        create_plot(time-timemax, SNR, pcolor, title = det+' matched filter SNR around event',
            plot_label=det+' SNR(t)', xlabel='Time since {0:.4f}'.format(timemax), ylabel='SNR')
        # zoom in
        plt.subplot(2,1,2)
        create_plot(time-timemax, SNR, pcolor, plot_label=det+' SNR(t)', xlabel='Time since {0:.4f}'.format(timemax), ylabel='SNR',
            xlim=[-0.15,0.05])
        #plt.xlim([-0.3,+0.3])
        ...
        plt.figure(figsize=(10,8))
        plt.subplot(2,1,1)
        create_plot(time-tevent, strain_whitenbp, pcolor, plot_label=det+' whitened h(t)')
        create_plot(time-tevent, template_match, 'k', title=det+' whitened data around event', plot_label='Template(t)', 
            xlabel='Time since {0:.4f}'.format(timemax), ylabel='whitened strain (units of noise stdev)',
            xlim=[-0.15,0.05], ylim=[-10,10])
    
        plt.subplot(2,1,2)
        create_plot(time-tevent,strain_whitenbp-template_match,pcolor,
         title=det+' Residual whitened data after subtracting template around event', plot_label=det+' resid',
         xlabel='Time since {0:.4f}'.format(timemax), ylabel='whitened strain (units of noise stdev)',
         xlim=[-0.15,0.05], ylim=[-10,10])
        ...
        plt.figure(figsize=(10,6))
        create_plot(datafreq, template_f, 'k', plot_label='template(f)*sqrt(f)', loglog=True)
        create_plot(freqs, np.sqrt(data_psd), pcolor, title=det+' ASD and template around event', plot_label=det+' ASD',
            xlabel='frequency (Hz)', ylabel='strain noise ASD (strain/rtHz), template h(f)*rt(f)', 
            xlim=[20, fs/2], ylim=[1e-24, 1e-20], loglog=True)
